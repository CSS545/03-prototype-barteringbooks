...
const AddPost = ({ navigation }) => {
        ...
        if (post != null) {
            AsyncStorage.getItem('user')
                .then(data => {
                    ...
                    fetch('http://10.156.11.112:3000/addpost', {
                        method: 'post',
                        headers: {
    ...
    return (
        <View style={containerFull}>
            ...
            {/*  */}
            <Text style={formHead2}>Categories</Text>
            <TextInput placeholder="Write One Category from below" style={formInput}
                onChangeText={(text) => setpostdescription(text)}
                multiline={true}
                numberOfLines={5} 
            />
            <Categories/>
            {/* <Categories> <Text style={formbtn} onPress={() => setcategories()}></Text></Categories> */}
            {
